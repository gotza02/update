...
# Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Izz Store ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Author : ${green}SABDO STORE ® ${NC}${YELLOW}(${NC} ${green} SPS ${NC}${YELLOW})${NC}"
echo -e " © Recode By Yinn Store ${YELLOW}(${NC} 2024 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
...
export date=$(date -d "0 days" +"%d-%m-%Y - %X")
export OS_Name=$(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g' | tr -d '"')
...
first_setup(){
    ...
    if [[ $(grep -w ID /etc/os-release | head -n1 | sed 's/ID=//g') == "ubuntu" ]]; then
        echo "Setup Dependencies $(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g')"
        sudo apt update -y
        apt-get install --no-install-recommends software-properties-common
        add-apt-repository ppa:vbernat/haproxy-2.0 -y
        apt-get -y install haproxy=2.0.\*
    elif [[ $(grep -w ID /etc/os-release | head -n1 | sed 's/ID=//g') == "debian" ]]; then
        echo "Setup Dependencies For OS Is $(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g')"
        ...
        echo -e " Your OS Is Not Supported ($(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g') )"
        exit 1
    fi
}
...
nginx_install() {
    # Checking System
    if [[ $(grep -w ID /etc/os-release | head -n1 | sed 's/ID=//g') == "ubuntu" ]]; then
        print_install "Setup nginx For OS Is $(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g')"
        sudo apt-get install nginx -y 
    elif [[ $(grep -w ID /etc/os-release | head -n1 | sed 's/ID=//g') == "debian" ]]; then
        print_install "Setup nginx For OS Is $(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g')"
        apt -y install nginx 
    else
        echo -e " Your OS Is Not Supported ( ${YELLOW}$(grep -w PRETTY_NAME /etc/os-release | head -n1 | sed 's/PRETTY_NAME=//g')${NC} )"
        exit 1
    fi
}
...
setup_domain() {
    ...
    echo -e "     \e[1;32m2)\e[0m Use Random Domain Specific to Digital Ocean ISP OTHERS ✖️ "
    echo -e "   ------------------------------------"
...
install_xray() {
    ...
    print_install "Installing Configuration Packages"
...
# Install and configure the limit quota service
udp_mini() {
    clear
